...
app.get('/search',function(req,res){
    console.log(req.query) //get요청 값 받기
    //  /abc/ abc가 포함된걸 다 찾음 but db내용이 많으면 느림
    ...
    //      console.log(결과);
    //      res.render('search.ejs',{posts:결과});
    //      //단점: 띄어쓰기안된 한국어는 검색 안됨
    //      //1. 날짜별로 1000개씩 정도로 구간별로 검색
    //      //2. text인덱스 만들때 다른 알고리즘으로 사용(ex)nGRAM
    //      //3. 몽고디비에서는 search index
    // })
    // 몽고디비 search index 사용
    var 검색조건 = [
        {
          $search: {
            index: 'titleSearch',
